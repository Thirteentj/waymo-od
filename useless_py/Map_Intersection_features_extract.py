    ...
    for lane_in_id in all_lane_id:
        if (lane_in_id not in lane_turn_left_id) and (lane_in_id not in lane_turn_right_id):
            if judge_lane_in_intersection(single_map_dict[lane_in_id], points_key_new_plus) == 1:
                df_all_lane_topo_info.loc[
                    df_all_lane_topo_info['lane_id'] == lane_in_id, 'entry_or_exit'] = 'inside'  # 车道位于交叉口内部
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(file_index_list))):
            ...
            if intersection_center_loc_list != []:
                ...
                try:
                    all_lane_entry_exit_info, single_map_dict, lane_turn_left_id, lane_turn_right_id = map_topo_info_extract(
                        map_features)  # 车道拓扑关系提取

                    # road_edge_count, lane_count, road_line, all_element_count = plot_top_view_single_pic_map(scenario_trj,file_index,scenario_label,1,scenario,lane_turn_left_id,lane_turn_right_id,intersection_center_loc,length)#仅供测试使用
                    # df_single_seg_single_scenario_lane_topo_info = pd.DataFrame(single_scenario_all_lane_entry_exit_info)
                    # outpath_lane_topo_info_single = 'data/all_lane_topo_info/' + file_index + '_'+ str(scenario_label) +'_all_lane_topo_info' + '.csv'  # 现在是一个scenario导出一个csv,后面需要导出一个完整的
        ...
        if segment_count!= 0 and segment_count % 100 == 0:
            # -----------------ouput------------------
            outpath_lane_topo_info = 'E:/Result_save/data_save/all_lane_topo_info/'+ str(segment_count-100) +'_'+ str(segment_count) + '_all_seg_all_lane_topo_info.csv'  # 现在是一个scenario导出一个csv,后面需要导出一个完整的
            # print(outpath_lane_topo_info)
            ...
            outpath_intersection_info = 'E:/Result_save/data_save/' + str(segment_count-100) +'_'+ str(segment_count)+ '_all_intersection_info' + '.csv'  # 现在是一个scenario导出一个csv,后面需要导出一个完整的
